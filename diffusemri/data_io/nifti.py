...
def load_nifti_mask(filepath: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    # Optional: Check if the original data (before casting to bool) was binary {0,1}
    # For example, one could load with original dtype, check values, then cast.
    # For now, relying on the bool casting.
...
def load_fsl_bvecs(filepath: str) -> np.ndarray:
    ...
    try:
        # np.loadtxt can handle space or comma delimiters if they are consistent
    ...
    if bvecs.shape[0] == 3 and bvecs.shape[1] != 3:
        ...
        # Already Nx3, do nothing
        pass
    elif bvecs.shape[0] == 3 and bvecs.shape[1] == 3:
        # Ambiguous 3x3 case. Assume it's Nx3 (N=3).
        # If it was meant to be 3xN (3 vectors), this is consistent.
        # If it was meant to be 3 vectors of N=3, this is also consistent.
        # Dipy's gradient_table will later validate consistency with bvals length.
        pass
    else: # Other shapes are invalid
        raise ValueError(
            f"b-vectors in {filepath} must be 3xN or Nx3. Got shape {bvecs.shape}."
        )
...
def load_fsl_bvals(filepath: str) -> np.ndarray:
    ...
    # Ensure bvals is 1D
    bvals = bvals.squeeze() # Remove any redundant dimensions (e.g. from a single row/column matrix)
    if bvals.ndim == 0: # If squeeze results in a scalar (single b-value)
...
def load_nifti_study(
    ...
    # This check is also implicitly done by create_gradient_table,
    # but an early check can provide a clearer error source.
    if len(bvals) != len(bvecs):
        raise ValueError(
            f"Number of b-values ({len(bvals)}) does not match "
            f"number of b-vectors ({len(bvecs)})."
        )
    ...
    if mask_path is not None:
        ...
        if not np.allclose(affine, mask_affine, atol=1e-5): # Using a small tolerance for float comparison
            print(
                f"Warning: DWI affine and mask affine are different.\n"
                f"DWI affine:\n{affine}\n"
                f"Mask affine:\n{mask_affine}\n"
                f"Using DWI affine for all spatial interpretations."
            )
            # Decision to use DWI affine is implicit as `affine` is what's returned.
            # No direct action needed here unless we wanted to re-orient one image.
